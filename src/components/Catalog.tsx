...
const Catalog = () => {
  ...
  const [data] = useState(Albums); // хуки - функции которые помогают обрабатывать события в js
  ...
  //showedAlbums have a key named col
  const sorting = (col: keyof (typeof showedAlbums)[0]) => {
    if (order === "ASC") {
      const sorted = [...showedAlbums].sort(
        (a, b) =>
          (a[col] as String).toLowerCase() > (b[col] as String).toLowerCase()
            ? 1
            : -1 //если a[col]>b[col] то эл-ты меняются местами
      );
    ...
    if (order === "DSC") {
      const sorted = [...showedAlbums].sort((a, b) =>
        //typescript doesn't know that a[col] is string
        (a[col] as String).toLowerCase() < (b[col] as String).toLowerCase()
          ? 1
          : -1
      );
  ...
  let genresList: React.ReactNode[] = [];
    ...
    return data.filter((album) => {
      return (
        ...
        (selectedGenres.length > 0
          ? selectedGenres.includes(album.genres.genre1) ||
            selectedGenres.includes(album.genres.genre2) ||
            selectedGenres.includes(album.genres.genre3)
          : true)
      );
    });
  };
  ...
  const handlePageClick = (data: PaginationTypes) => {
